...
- A reserved `_private_steps` row, `pgconductor.internal.event-fanout.v1`, is the atomic fan-out commit marker. Destination insertion and marker insertion share one claim-fenced statement. A retry that sees the marker completes without re-reading subscriptions, including zero-destination events.
...
- Dispatch performs operator-specific indexed anchor probes, including bounded literal event-prefix expansion, unions fallback groups, fully verifies every normalized clause, deduplicates subscriptions, and inserts one destination per match. No JSONB predicate values, stored match counts, hashes, or residual JSON filter evaluation are used.
- TypeScript owns event catalog policy, limits, canonical field ordering, and type-sensitive scalar deduplication. The private registration SQL converts the JSON transport value into normalized relational rows set-wise.
