...
pub fn rebuild_index(subs: Vec<Subscription>) {
    ...
    for sub in subs {
        match &sub {
            ...
            Subscription::Event(event_sub) => {
                event_map
                    .entry(event_sub.event_key.clone())
                    .or_default()
                    .push(Arc::new(event_sub.clone()));
    ...
    let event_subs = event_map
        .into_iter()
        .map(|(k, v)| (k, Arc::from(v.into_boxed_slice())))
        .collect();

    SUBSCRIPTIONS.store(Some(Arc::new(SubscriptionIndex {
        db_subs,
        event_subs,
        by_id,
    })));
...
pub fn match_db_subs(schema: &str, table: &str, op: Operation) -> Option<Arc<[Arc<DbSubscription>]>> {
    let guard = SUBSCRIPTIONS.load();
    let idx = guard.as_deref()?;
    idx.db_subs
        .get(&(Arc::from(schema), Arc::from(table), op))
        .cloned()
}
