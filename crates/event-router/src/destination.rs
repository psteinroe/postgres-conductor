...
impl EventRouterDestination {
    ...
    fn filter_payload_columns(&self, payload: &serde_json::Value, filter_cols: &[Arc<str>]) -> serde_json::Value {
        ...
        if let Some(old) = obj.get("old").and_then(|v| v.as_object()) {
            let filtered: serde_json::Map<String, serde_json::Value> = old.iter()
        ...
        if let Some(new) = obj.get("new").and_then(|v| v.as_object()) {
            let filtered: serde_json::Map<String, serde_json::Value> = new.iter()
                .filter(|(k, _)| filter_cols.iter().any(|c| c.as_ref() == k.as_str()))
                .map(|(k, v)| (k.clone(), v.clone()))
                .collect();
