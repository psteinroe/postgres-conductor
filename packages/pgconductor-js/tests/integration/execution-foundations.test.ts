...
describe("execution foundations", () => {
	...
	test("uses enqueue position to break priority and run-at ties", async () => {
		const db = await database();
		await db.client.registerWorker({
	...
	test("fences stale completion, failure, and release results after recovery and re-claim", async () => {
		...
		expect(currentClaim.claim_token).not.toBe(oldClaim.claim_token);
		...
		>`
			select completed_at, failed_at, locked_by, claim_token
			from pgconductor._private_executions
			where id = ${executionId}::uuid
		`;
		...
		expect(untouched[0]?.claim_token).toBe(currentClaim.claim_token);
		...
		const settled = await db.sql<
			{ completed_at: Date | null; locked_by: string | null; claim_token: string | null }[]
		>`
			select completed_at, locked_by, claim_token
			from pgconductor._private_executions
			where id = ${executionId}::uuid
		`;
		expect(settled[0]?.completed_at).not.toBeNull();
		expect(settled[0]?.locked_by).toBeNull();
		expect(settled[0]?.claim_token).toBeNull();
	});
});
