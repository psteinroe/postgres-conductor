...
export class TestDatabase {
	...
	/**
	 * Helper object for time manipulation in tests.
	 */
	public readonly client = {
		setFakeTime: async ({ date }: { date: Date }): Promise<void> => {
			await this.sql.unsafe(`set pgconductor.fake_now = '${date.toISOString()}'`);
		},
		clearFakeTime: async (): Promise<void> => {
			await this.sql.unsafe(`set pgconductor.fake_now = ''`);
		},
	};
